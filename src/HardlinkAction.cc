...
HardlinkAction::HardlinkAction( const std::string& path )
	:
	path_( path )
{
	return ;
...
bool HardlinkAction::operator()( const Product& product )
	throw()
{
	...
	if( 0 != link( rpm->getFile().c_str() , buf_.str().c_str() ) ){
